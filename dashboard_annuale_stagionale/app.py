...
@st.cache_data
def load_raw_data(file_path="./csv_files/aggregation_per_season_anno.csv"):
    """
...
@st.cache_data
def load_data(directory_path="./csv_files", season="Winter"):
    """
    Carica i dati di clusterizzazione e dei centroidi per una stagione specifica.
    """
    years = list(range(1995, 2025))
    df_clusters_anni = {}
    cluster_means_anni = {}
    for year in years:
        df_clusters_anni[year] = pd.read_csv(directory_path + f"/clusterization_{season.lower()}_{year}.csv")
        cluster_means_anni[year] = pd.read_csv(directory_path + f"/clusterization_{season.lower()}_means_{year}.csv")
...
def plot_world_slider_streamlit_go(df, years=list(range(1995,2025)), n_clusters=5, method="K-Means", season="Winter"):
    """
    ...
    fig = go.Figure(
        ...
        layout=go.Layout(
            title=dict(
                text=f"Climate Clusters {season} {years[0]} - {years[-1]} ({method})",
                x=0.5,
                xanchor="center",
                font=dict(size=20)
            ),
